...
public:
    ...
    void generarLineal(std::vector<int32_t>& A, size_t n, int32_t epsilon) {
        if (epsilon < 1) epsilon = 1;            // evita division por cero en rand()%epsilon
        A.resize(n);
        if (n == 0) return;
        A[0] = rand() % 100;
    ...
    // Distribucion normal (gaussiana). Se genera, se ordena y se desplaza para
    // garantizar valores >= 0 (requisito de Gap-Coding y Elias-Fano).
    ...
    // Busqueda binaria estandar sobre el arreglo explicito.
